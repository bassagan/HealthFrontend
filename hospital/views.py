...
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_bug1(request):
    patients=models.Patient.objects.all().filter(status=True)
    return render(request,'hospital/admin_bug1.html',{'patients':patients})

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_bug2(request):
    patients=models.Patient.objects.all().filter(status=True)
    return render(request,'hospital/admin_bug2.html',{'patients':patients})

@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_bug3(request):
    patients=models.Patient.objects.all().filter(status=True)
    return render(request,'hospital/admin_bug3.html',{'patients':patients})
